...
async function main() {
  ...
  // Cross-connects. MRC/NRC stored in minor units (cents) like invoices.
  await db.insert(crossConnects).values([
    {
      orgId: uniOrg.id,
      fromCabinetId: uniA12.id,
      toLabel: "MMR rack 3, port 12",
      connectionType: "carrier",
      zSideProvider: "Liquid Telecom",
      speedGbps: 10,
      media: "fiber_sm",
      status: "provisioned",
      installFeeMinor: 25_000_00,
      monthlyChargeMinor: 18_000_00,
      requestedBy: uniAdmin.id,
    ...
    },
    {
      ...
      media: "fiber_sm",
      status: "provisioned",
      installFeeMinor: 15_000_00,
      monthlyChargeMinor: 12_000_00,
      requestedBy: uniAdmin.id,
      provisionedAt: new Date(Date.now() - 30 * 24 * 3600_000),
    },
    {
      orgId: uniOrg.id,
      fromCabinetId: uniA13.id,
      toLabel: "AWS Direct Connect — Cage 2, panel 7",
      connectionType: "cloud",
      zSideProvider: "AWS",
      speedGbps: 10,
      media: "fiber_sm",
      status: "pending",
      notes: "LOA/CFA reference DX-2026-0142. Needed before quarter close.",
      requestedBy: uniAdmin.id,
    },
    {
      orgId: fintechOrg.id,
      fromCabinetId: fintechB07.id,
      toLabel: "MMR rack 1, port 4",
      connectionType: "carrier",
      zSideProvider: "Safaricom",
      speedGbps: 1,
      media: "fiber_sm",
      status: "provisioned",
      installFeeMinor: 20_000_00,
      monthlyChargeMinor: 9_500_00,
      requestedBy: fintechAdmin.id,
